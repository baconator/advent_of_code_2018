...
#[test]
fn test_d4h() {
    let solutions = vec![("[1518-11-01 00:00] Guard #10 begins shift
...
pub fn solve(lines: impl Iterator<Item=String>) -> u32 {
    ...
    guards_and_minutes_to_time_asleep
        .sort_by_key(|(_, minutes_to_time_asleep)| {
            minutes_to_time_asleep.values().sum::<usize>()
        });

    let (guard_id, minutes_to_time_asleep) = guards_and_minutes_to_time_asleep
        .iter()
        .max_by_key(|(_, minutes_to_time_asleep)| {
            minutes_to_time_asleep.values().max().unwrap()
        }).unwrap();

    println!("Guard: {:#?}, mtta: {:#?}", guard_id, minutes_to_time_asleep);
    let (sleepiest_minute, _) = minutes_to_time_asleep
        .iter()
        .max_by_key(|t| t.1)
        .unwrap();
